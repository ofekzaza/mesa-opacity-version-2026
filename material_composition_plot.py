...
ISO_LABELS = {
    'h1': r'H', 'he3': r'$^3$He', 'he4': r'$^4$He',
    'c12': r'$^{12}$C', 'n14': r'$^{14}$N', 'o16': r'$^{16}$O',
    'ne20': r'$^{20}$Ne', 'mg24': r'$^{24}$Mg', 'si28': r'$^{28}$Si',
}
ISO_COLORS = {
    'h1': '#1f77b4', 'he3': '#aec7e8', 'he4': '#ff7f0e',
    'c12': '#2ca02c', 'n14': '#d62728', 'o16': '#9467bd',
    'ne20': '#8c564b', 'mg24': '#e377c2', 'si28': '#7f7f7f',
}
...
def _get_last_profile_num(path):
    try:
        ...
        import glob
        prof_files = sorted(glob.glob(f"{path}/profile*.data"),
...
def _plot_abundance_vs_logr(mass: int, name: str, phase: str = ""):
    ...
    if x_data is not None:
        for iso in valid_isos:
            ax.plot(x_data, getattr(prof, iso),
...
def _plot_abundance_evolution(mass: int, name: str, phase: str = ""):
    ...
    fig, ax = plt.subplots(figsize=(8, 6))
    for iso in valid_isos:
        ax.plot(model_numbers, iso_avg[iso],
               label=ISO_LABELS.get(iso, iso),
               color=ISO_COLORS.get(iso, '#333333'),
               linewidth=1.2)
